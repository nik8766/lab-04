...
def generate_invalid_user_report():
    ...
    Invalid_data = log_analysis_lib.filter_log_by_regex(log_path, r'^(.+ \d+) (.{8}).*(\w+).*SRC=(.*?) ')[1]

    # Generate the CSV report
    df = pd.DataFrame(Invalid_data)
    filename = f'invalid_users.csv'
    headings = ('Date', 'Time', 'Username', 'IP Address')
    df.to_csv(filename, index=True, header=headings)
    return
...
def generate_source_ip_log(ip_address):
    ...
    Source_IP = log_analysis_lib.filter_log_by_regex(log_path, r'^(.+ \d+) (.{8}.) SRC=(.*?) DST=(.*?) SPT=(.*?) DPT=(.*?) LEN=(.*?) ')[0]

    # Save all records to a plain text .log file
    df = pd.DataFrame(Source_IP)
    filename = f'source_ip_{re.sub(r".","_", ip_address)}.log'

    df.to_csv(filename)
    
    with open(filename, 'w') as file:

        for record in Source_IP:
            file.write(record + '\n')
    return
